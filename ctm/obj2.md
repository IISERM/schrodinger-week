...
function simulate(initial_positions, D, r, d, kT, steps=1e5)
    ...
    plotscene = scatter(pos[:,1], pos[:,2], pos[:,3])
    xlims!(plotscene, (-150, 150))
    ylims!(plotscene, (-150, 150))
    zlims!(plotscene, (-150, 150))
    record(plotscene, "file.mp4") do io
        for i in 1:steps
            old_pos = copy(pos)
            old_energy = energy
            pos[rand(1:num_particles), :] += d * (2 * rand(3) .- 1)
            ...
            if (i % (steps/1000) == 0)
                ...
                scatter!(plotscene, pos[:,1], pos[:,2], pos[:,3])
                recordframe!(io)
            end
        end
    end
